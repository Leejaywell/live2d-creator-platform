/**
 * Assign the four bundled Azur Lane demo characters to two creator accounts:
 *
 *   - 爱宕 (aidang_2)            -> the seeded test creator ("creator"), replacing
 *                                  whatever model that account's project had. Fully
 *                                  wired: model + voices + expressions + motions.
 *   - 圣路易斯 / 贝尔法斯特 / 埃吉尔 -> a new creator account ("azurlane").
 *
 * Each model dir under public/live2d/<key> is zipped and pushed through the real
 * upload pipeline (validate -> extract -> store), so the projects behave exactly
 * like a creator-uploaded model. Voices point at the static /live2d/audio files;
 * motions + expressions come from the model3.json itself (爱宕 ships authored
 * expressions, the others rely on motions only).
 *
 * Idempotent: re-running re-uploads models and rebuilds voices/tags; fan codes
 * are only generated when a project has none.
 *
 * Run: npm run setup:creator-models
 */
...
const TEST_CREATOR_USERNAME = (process.env.SEED_CREATOR_USERNAME || "creator").toLowerCase();
const NEW_CREATOR_USERNAME = (process.env.AZURLANE_USERNAME || "azurlane").toLowerCase();
const NEW_CREATOR_PASSWORD = process.env.AZURLANE_PASSWORD || "ChangeMe123!";
...
async function ensureActivePlan(creatorId: string, maxProjects: number) {
  const planData = {
    planName: "Demo",
    tier: "paid" as const,
...
// Wire a character onto a project: model upload, voices, trigger tags, fan codes, publish.
async function configureCharacter(input: {
  creatorId: string;
  projectId: string;
  character: CharacterSeed;
  audioConfig: AudioConfig;
}) {
  const { creatorId, projectId, character, audioConfig } = input;
  const dir = path.join(PUBLIC_LIVE2D, character.key);

  // Model: zip the bundled dir and push it through the real upload pipeline.
  ...
  // Voices from audio_config.json (static /live2d/audio files).
  await prisma.voiceAsset.deleteMany({ where: { projectId } });
  ...
  // Trigger tags (>=1 enabled so the project can publish + drive chat reactions).
  // Only 爱宕 ships authored expressions; the others carry no live2dExpression.
  await prisma.triggerTag.deleteMany({ where: { projectId } });
  const expr = character.hasExpressions
  ...
  // Fan codes (only if none yet) so the audience chat gate works.
...
async function main() {
  ...
  // ---- 1. 爱宕 -> existing test creator ("creator"), replacing its model. ----
  const testCreator = await prisma.user.findUnique({ where: { username: TEST_CREATOR_USERNAME } });
  if (!testCreator) {
    throw new Error(`Test creator '${TEST_CREATOR_USERNAME}' not found — run 'npm run db:seed' first.`);
  }
  await ensureActivePlan(testCreator.id, 4);

  // Reuse the creator's existing project (the seeded one) so we "replace" its
  // model in place; otherwise create a fresh atago project.
  const existing =
    (await prisma.project.findFirst({ where: { creatorId: testCreator.id, slug: ATAGO.slug } })) ??
    (await prisma.project.findFirst({ where: { creatorId: testCreator.id } }));
  const atagoProject = existing
    ? await prisma.project.update({
        ...
        data: {
          ...
          name: ATAGO.name,
          intro: ATAGO.intro,
          characterSetting: ATAGO.characterSetting,
          systemPrompt: ATAGO.systemPrompt,
          welcomeMessage: ATAGO.welcomeMessage,
          theme: ATAGO.theme,
        },
      })
  ...
  await configureCharacter({
    creatorId: testCreator.id,
    projectId: atagoProject.id,
    character: ATAGO,
    audioConfig,
  });
  console.log(`Test creator '${TEST_CREATOR_USERNAME}' now owns 爱宕 (/c/${ATAGO.slug}).\n`);

  // ---- 2. The other three -> a new creator account ("azurlane"). ----
  const passwordHash = await hashPassword(NEW_CREATOR_PASSWORD);
  const newCreator = await prisma.user.upsert({
    where: { username: NEW_CREATOR_USERNAME },
    update: { role: "creator", status: "active", passwordHash },
    create: {
      username: NEW_CREATOR_USERNAME,
      email: internalEmailForUsername(NEW_CREATOR_USERNAME),
      role: "creator",
      status: "active",
      emailVerified: new Date(),
      passwordHash,
      creatorProfile: { create: { displayName: "碧蓝航线 角色馆", bio: "碧蓝航线演示角色合集" } },
    },
  });
  await prisma.creatorProfile.upsert({
    where: { userId: newCreator.id },
    update: { displayName: "碧蓝航线 角色馆", bio: "碧蓝航线演示角色合集" },
    create: { userId: newCreator.id, displayName: "碧蓝航线 角色馆", bio: "碧蓝航线演示角色合集" },
  });
  await ensureActivePlan(newCreator.id, OTHERS.length);
  ...
  console.log(
    `\nNew creator '${NEW_CREATOR_USERNAME}' (password: ${NEW_CREATOR_PASSWORD}) now owns ${OTHERS.length} published characters.`,
  );
}
...
main()
  ...
  .finally(async () => {
    ...
    // PGlite keeps the event loop alive after disconnect, so force exit to
    // release the single-process data-dir lock.
